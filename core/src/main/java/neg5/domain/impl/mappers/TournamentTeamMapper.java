...
public class TournamentTeamMapper extends AbstractObjectMapper<TournamentTeam, TournamentTeamDTO> {
    ...
    protected void enrichDTO(TournamentTeamDTO tournamentTeamDTO, TournamentTeam tournamentTeam) {
        tournamentTeamDTO.setDivisions(
                tournamentTeam.getDivisions() == null
                        ? new HashSet<>()
                        : tournamentTeam.getDivisions().stream()
                                .map(divisionMapper::toDTO)
                                .collect(Collectors.toSet()));
        tournamentTeamDTO.setPlayers(
                tournamentTeam.getPlayers() == null
                        ? new HashSet<>()
    ...
    protected void addMappings() {
        getEntityToDTOTypeMap()
                .addMappings(
                        mp -> {
                            mp.skip(TournamentTeamDTO::setDivisions);
                            mp.skip(TournamentTeamDTO::setPlayers);
                        });
