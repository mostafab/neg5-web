...
public class TournamentMatchDAO extends AbstractDAO<TournamentMatch, String> {
    ...
    private static final String QUERY =
            "SELECT match_information.*, COALESCE(match_players.players, '{}') as players, COALESCE(match_phases.phases, '{}') as phases\n"
                    + "FROM\n"
                    + "\n"
                    + "(\n"
                    + "    SELECT\n"
                    ...
                    + "                'forfeit', player_team_join.forfeit,\n"
                    + "                'bouncebackPoints', player_team_join.bounceback_points,\n"
                    + "                'overtimeTossups', player_team_join.overtime_tossups_gotten\n"
                    + "            )\n"
                    + "        ) as teams\n"
                    + "\n"
                    + "        FROM\n"
                    ...
                    + "\n"
                    + "            GROUP BY TTM.tournament_id, TTM.team_id, TTM.match_id, TTM.score, TTM.bounceback_points, TTM.overtime_tossups_gotten, TTM.forfeit\n"
                    + "\n"
                    + "        ) as player_team_join\n"
                    + "\n"
    ...
    public List<String> findMatchIdsByTournament(String tournamentId) {
        return getEntityManager()
                .createQuery(
                        "SELECT m.id from TournamentMatch m WHERE m.tournament.id = :tournamentId",
                        String.class)
                .setParameter("tournamentId", tournamentId)
                .getResultList();
    }
