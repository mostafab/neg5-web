...
public class AbstractObjectMapper<Entity, DTO> {
    ...
    private void copyEntitiesToIds(Entity source, DTO target) {
        ...
        idEntityProps.stream()
                // Copy over the property if it's an id data object, and the target has a
                // corresponding id
                        ...
                        .filter(
                                property ->
                                        PropertyUtils.isWriteable(
                                                target, getIdPropertyName(property)))
                        .forEach(
                                property -> {
                                    try {
                                        IdDataObject dataObject =
    ...
    private void copyIdsToEntities(DTO source, Entity target) {
                ...
                .filter(
                        propertyDescriptor ->
                                PropertyUtils.isReadable(
                                        source, getIdPropertyName(propertyDescriptor)))
                .forEach(
                        property -> {
                            try {
                                Object id =
                                        PropertyUtils.getSimpleProperty(
                                                source, getIdPropertyName(property));
                                IdDataObject idDataObject = null;
                                Constructor<?> constructor =
                                        property.getPropertyType().getDeclaredConstructor();
                                if (id != null
                                        && constructor != null
                                        && id instanceof Serializable) {
                                    // This allows us to use non-public default constructors, can be
                                    // dangerous, but is needed
        ...
        if (target instanceof CompositeIdObject) {
            List<PropertyDescriptor> compositeIdProperties =
                    getCompositeIdPropertiesOfEntity((CompositeIdObject) target);
    ...
    private void copyDtoIdsToCompositeIdEntities(CompositeId idInstance, DTO source) {
        List<PropertyDescriptor> idDataObjProperties =
                getCompositeIdDataObjectProperties(idInstance);
        for (PropertyDescriptor idDataObjProp : idDataObjProperties) {
            try {
                Object id =
                        PropertyUtils.getSimpleProperty(source, getIdPropertyName(idDataObjProp));
                if (id != null) {
                    Constructor<?> propConstructor =
                            idDataObjProp.getPropertyType().getDeclaredConstructor();
    ...
    private List<PropertyDescriptor> getIdEntityProperties(@Nonnull Entity entity) {
        ...
        return Lists.newArrayList(PropertyUtils.getPropertyDescriptors(entity)).stream()
                .filter(
                        propertyDescriptor ->
                                IdDataObject.class.isAssignableFrom(
                                        propertyDescriptor.getPropertyType()))
                .filter(
                        propertyDescriptor ->
                                !ignoredPropNames.contains(propertyDescriptor.getName()))
                .collect(Collectors.toList());
    }

    private List<PropertyDescriptor> getCompositeIdDataObjectProperties(
            @Nonnull CompositeId compositeIdObject) {
    ...
    private List<PropertyDescriptor> getCompositeIdPropertiesOfEntity(
            @Nonnull CompositeIdObject compositeIdObject) {
        return Lists.newArrayList(PropertyUtils.getPropertyDescriptors(compositeIdObject)).stream()
                .filter(
                        propertyDescriptor ->
                                CompositeId.class.isAssignableFrom(
                                        propertyDescriptor.getPropertyType()))
                .collect(Collectors.toList());
    }
}
