...
public class MatchToMatchDTOMapper extends AbstractObjectMapper<Match, TournamentMatchDTO> {
    ...
    public TournamentMatchDTO toDTO(
            Match match, Map<Integer, TournamentTossupValueDTO> tossupValues) {
                        ...
                        answer -> {
                            if (tossupValues.containsKey(answer.getTossupValue())) {
                                answer.setAnswerType(
                                        tossupValues.get(answer.getTossupValue()).getAnswerType());
                            }
                        });
